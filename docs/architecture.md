...
> владельца ADR-0002 и реплики диалога) прокидывается `AgentLoop` каждому
> инструменту; `ExecTool` его игнорирует. Добавлен инструмент `search_documents`
> (`application/search.py`: структурно реализует `Tool` над `RagService`, обёрнут
> в `MeteredTool`); перед поиском запрос переписывается в самостоятельный через
> порт rag-слоя `QueryRewriter` (`rag/rewrite.py`) — реализация `LlmQueryRewriter`
> поверх провайдера инференса (та же чат-модель) собирается в композиционном
> корне; сбой переписывания переводит поиск на сырой запрос, вызов переписывания
> учитывается метриками `llm_call` без содержимого. Эти строки и разделы про
> agent/rag в этом документе будут дополнены при финальной правке документации
> в тикете 07.
