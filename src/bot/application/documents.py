...
и очистка корпуса одного владельца сериализуются блокировкой (замена документа
атомарна, поиск во время индексации видит старый корпус, очистка не даёт
документу «воскреснуть» после отчёта о ней); телом работы занимается
RagService, здесь — только очередь владельца и типизированные сценарии. Чат не
...
class DocumentService:
    ...
    async def clear_documents(self, owner_id: TelegramUserId) -> int:
        """Очистить корпус владельца; вернуть число удалённых документов.

        Берёт блокировку владельца: очистка ждёт идущую индексацию, чтобы
        доехавший после очистки документ не «воскрес» в корпусе.
        """
